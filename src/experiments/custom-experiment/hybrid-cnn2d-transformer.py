...
class TemporalConvTransformer(nn.Module):
    def __init__(self, num_inputs, num_outputs, hidden_dim, n_head, transformer_layers, seq_len,
                 dropout=0.2):
        super(TemporalConvTransformer, self).__init__()
        self.conv = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(10,10), stride=1, padding="same")
    ...
    def forward(self, x):
        ...
        cnn_input = x[:, :, :, :self.split_point]
        # print(f"cnn_input shape: {x.shape}")
        transformer_input = x[:, :, :, self.split_point:]
        ...
        combined = torch.cat([cnn_output, transformer_input], dim=3)
...
def train_model_large(num_inputs, train_dl, hidden_dim, n_head, transformer_layers, dropout, lookback):
...
if __name__ == '__main__':
    ...
    # file_path = base_dir+'/data/stock-data/AMD.csv'
    # file_path = base_dir+"/data/iTransformer_datasets/exchange_rate/exchange_rate.csv"
    data = load_data(file_path, category="stock")
    ...
    # save the loss log
    np.save(base_dir+"/experiments/loss-data/traffic/TCformer_2d_train_loss_log.npy", train_loss_log)
    np.save(base_dir+"/experiments/loss-data/traffic/TCformer_2d_test_loss_log.npy", test_loss_log)
